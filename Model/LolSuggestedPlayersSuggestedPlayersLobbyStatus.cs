/* 
 * LeagueClient
 *
 * 7.23.209.3517
 *
 * OpenAPI spec version: 1.0.0
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

using System;
using System.Linq;
using System.Text;
using System.Collections.Generic;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using System.ComponentModel.DataAnnotations;


namespace LeagueClientApi.Model
{
    /// <summary>
    /// LolSuggestedPlayersSuggestedPlayersLobbyStatus
    /// </summary>
    [DataContract]
    public partial class LolSuggestedPlayersSuggestedPlayersLobbyStatus :  IEquatable<LolSuggestedPlayersSuggestedPlayersLobbyStatus>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="LolSuggestedPlayersSuggestedPlayersLobbyStatus" /> class.
        /// </summary>
        /// <param name="InvitedSummonerIds">InvitedSummonerIds.</param>
        /// <param name="MemberSummonerIds">MemberSummonerIds.</param>
        /// <param name="QueueId">QueueId.</param>
        public LolSuggestedPlayersSuggestedPlayersLobbyStatus(List<long?> InvitedSummonerIds = default(List<long?>), List<long?> MemberSummonerIds = default(List<long?>), int? QueueId = default(int?))
        {
            this.InvitedSummonerIds = InvitedSummonerIds;
            this.MemberSummonerIds = MemberSummonerIds;
            this.QueueId = QueueId;
        }
        
        /// <summary>
        /// Gets or Sets InvitedSummonerIds
        /// </summary>
        [DataMember(Name="invitedSummonerIds", EmitDefaultValue=false)]
        public List<long?> InvitedSummonerIds { get; set; }

        /// <summary>
        /// Gets or Sets MemberSummonerIds
        /// </summary>
        [DataMember(Name="memberSummonerIds", EmitDefaultValue=false)]
        public List<long?> MemberSummonerIds { get; set; }

        /// <summary>
        /// Gets or Sets QueueId
        /// </summary>
        [DataMember(Name="queueId", EmitDefaultValue=false)]
        public int? QueueId { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class LolSuggestedPlayersSuggestedPlayersLobbyStatus {\n");
            sb.Append("  InvitedSummonerIds: ").Append(InvitedSummonerIds).Append("\n");
            sb.Append("  MemberSummonerIds: ").Append(MemberSummonerIds).Append("\n");
            sb.Append("  QueueId: ").Append(QueueId).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            return this.Equals(obj as LolSuggestedPlayersSuggestedPlayersLobbyStatus);
        }

        /// <summary>
        /// Returns true if LolSuggestedPlayersSuggestedPlayersLobbyStatus instances are equal
        /// </summary>
        /// <param name="other">Instance of LolSuggestedPlayersSuggestedPlayersLobbyStatus to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(LolSuggestedPlayersSuggestedPlayersLobbyStatus other)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            if (other == null)
                return false;

            return 
                (
                    this.InvitedSummonerIds == other.InvitedSummonerIds ||
                    this.InvitedSummonerIds != null &&
                    this.InvitedSummonerIds.SequenceEqual(other.InvitedSummonerIds)
                ) && 
                (
                    this.MemberSummonerIds == other.MemberSummonerIds ||
                    this.MemberSummonerIds != null &&
                    this.MemberSummonerIds.SequenceEqual(other.MemberSummonerIds)
                ) && 
                (
                    this.QueueId == other.QueueId ||
                    this.QueueId != null &&
                    this.QueueId.Equals(other.QueueId)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            // credit: http://stackoverflow.com/a/263416/677735
            unchecked // Overflow is fine, just wrap
            {
                int hash = 41;
                // Suitable nullity checks etc, of course :)
                if (this.InvitedSummonerIds != null)
                    hash = hash * 59 + this.InvitedSummonerIds.GetHashCode();
                if (this.MemberSummonerIds != null)
                    hash = hash * 59 + this.MemberSummonerIds.GetHashCode();
                if (this.QueueId != null)
                    hash = hash * 59 + this.QueueId.GetHashCode();
                return hash;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}

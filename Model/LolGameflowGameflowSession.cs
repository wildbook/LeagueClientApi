/* 
 * LeagueClient
 *
 * 7.23.209.3517
 *
 * OpenAPI spec version: 1.0.0
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

using System;
using System.Text;
using System.Collections.Generic;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using System.ComponentModel.DataAnnotations;


namespace LeagueClientApi.Model
{
    /// <summary>
    /// LolGameflowGameflowSession
    /// </summary>
    [DataContract]
    public partial class LolGameflowGameflowSession :  IEquatable<LolGameflowGameflowSession>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="LolGameflowGameflowSession" /> class.
        /// </summary>
        /// <param name="GameClient">GameClient.</param>
        /// <param name="GameData">GameData.</param>
        /// <param name="GameDodge">GameDodge.</param>
        /// <param name="Map">Map.</param>
        /// <param name="Phase">Phase.</param>
        public LolGameflowGameflowSession(LolGameflowGameflowGameClient GameClient = default(LolGameflowGameflowGameClient), LolGameflowGameflowGameData GameData = default(LolGameflowGameflowGameData), LolGameflowGameflowGameDodge GameDodge = default(LolGameflowGameflowGameDodge), LolGameflowGameflowGameMap Map = default(LolGameflowGameflowGameMap), LolGameflowGameflowPhase Phase = default(LolGameflowGameflowPhase))
        {
            this.GameClient = GameClient;
            this.GameData = GameData;
            this.GameDodge = GameDodge;
            this.Map = Map;
            this.Phase = Phase;
        }
        
        /// <summary>
        /// Gets or Sets GameClient
        /// </summary>
        [DataMember(Name="gameClient", EmitDefaultValue=false)]
        public LolGameflowGameflowGameClient GameClient { get; set; }

        /// <summary>
        /// Gets or Sets GameData
        /// </summary>
        [DataMember(Name="gameData", EmitDefaultValue=false)]
        public LolGameflowGameflowGameData GameData { get; set; }

        /// <summary>
        /// Gets or Sets GameDodge
        /// </summary>
        [DataMember(Name="gameDodge", EmitDefaultValue=false)]
        public LolGameflowGameflowGameDodge GameDodge { get; set; }

        /// <summary>
        /// Gets or Sets Map
        /// </summary>
        [DataMember(Name="map", EmitDefaultValue=false)]
        public LolGameflowGameflowGameMap Map { get; set; }

        /// <summary>
        /// Gets or Sets Phase
        /// </summary>
        [DataMember(Name="phase", EmitDefaultValue=false)]
        public LolGameflowGameflowPhase Phase { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class LolGameflowGameflowSession {\n");
            sb.Append("  GameClient: ").Append(GameClient).Append("\n");
            sb.Append("  GameData: ").Append(GameData).Append("\n");
            sb.Append("  GameDodge: ").Append(GameDodge).Append("\n");
            sb.Append("  Map: ").Append(Map).Append("\n");
            sb.Append("  Phase: ").Append(Phase).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            return this.Equals(obj as LolGameflowGameflowSession);
        }

        /// <summary>
        /// Returns true if LolGameflowGameflowSession instances are equal
        /// </summary>
        /// <param name="other">Instance of LolGameflowGameflowSession to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(LolGameflowGameflowSession other)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            if (other == null)
                return false;

            return 
                (
                    this.GameClient == other.GameClient ||
                    this.GameClient != null &&
                    this.GameClient.Equals(other.GameClient)
                ) && 
                (
                    this.GameData == other.GameData ||
                    this.GameData != null &&
                    this.GameData.Equals(other.GameData)
                ) && 
                (
                    this.GameDodge == other.GameDodge ||
                    this.GameDodge != null &&
                    this.GameDodge.Equals(other.GameDodge)
                ) && 
                (
                    this.Map == other.Map ||
                    this.Map != null &&
                    this.Map.Equals(other.Map)
                ) && 
                (
                    this.Phase == other.Phase ||
                    this.Phase != null &&
                    this.Phase.Equals(other.Phase)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            // credit: http://stackoverflow.com/a/263416/677735
            unchecked // Overflow is fine, just wrap
            {
                int hash = 41;
                // Suitable nullity checks etc, of course :)
                if (this.GameClient != null)
                    hash = hash * 59 + this.GameClient.GetHashCode();
                if (this.GameData != null)
                    hash = hash * 59 + this.GameData.GetHashCode();
                if (this.GameDodge != null)
                    hash = hash * 59 + this.GameDodge.GetHashCode();
                if (this.Map != null)
                    hash = hash * 59 + this.Map.GetHashCode();
                if (this.Phase != null)
                    hash = hash * 59 + this.Phase.GetHashCode();
                return hash;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
